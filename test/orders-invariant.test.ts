 ...
 *
 * Requires a migrated DATABASE_URL (a Neon branch). When the schema is absent or the DB
 * is unreachable, the tests self-skip so the default `bun test` run stays green.
 *
...
async function schemaReady(): Promise<boolean> {
  try {
    await db.select({ id: orders.id }).from(orders).limit(0)
    return true
  } catch {
    return false
  }
...
describe('orders: one OPEN order per table', () => {
  it('rejects a second OPEN order for the same table with a unique violation', async () => {
    if (!(await schemaReady())) {
      console.warn('[skip] orders invariant: schema not migrated on DATABASE_URL')
      return
    }
  ...
  it('allows a new OPEN order once the prior one is no longer OPEN', async () => {
