...
| Integration | `test/tables/table-use-cases.test.ts` (use-case behavior incl. create mints a non-empty `qrToken` and defaults `status=EMPTY`, update patches only sent fields and 404s a cross-tenant id, empty-patch returns the unchanged row, regenerate replaces the token, delete removes an empty table but refuses one with any order — proven with a PAID historical order — via `409 TABLE_IN_USE`) and `test/tables/tables-routes.integration.test.ts` (HTTP CRUD, RBAC 403/401, status+qrToken stripped on create, regenerate-qr old-token→404/new-token→200, cross-tenant `TABLE_NOT_FOUND` on PATCH/DELETE/regenerate, empty-PATCH body→400, delete-with-any-order→`409 TABLE_IN_USE`). Live Neon DB was reachable — DB suite ran rather than self-skipping. |
